...
function renderSlot(slotId) {
  ...
  newSlotElement.addEventListener('click', (e) => {
    e.stopPropagation();
    handleSlotClick(slotId, e.currentTarget);
  });
...
function renderAllSlots() {
  document
    ...
    .forEach((slotElement) => renderSlot(slotElement.id));
}
...
function clearCurrentSet() {
  if (
    confirm(`セット${activeSetId} の装備を全てクリアします。よろしいですか？`)
  ) {
    equippedItems = {};
    saveAllEquipmentSets();
    renderAllSlots();
...
async function calculateStatsForSet(items) {
  ...
  for (const slotId in items) {
    ...
    for (const statName of STAT_HEADERS) {
      ...
      if (value !== 0) {
        if (
          String(item[statName]).includes('%') ||
          (baseValue > 0 && baseValue < 1 && !Number.isInteger(baseValue))
        ) {
