...
auth.get('/google/callback', async (c) => {
    ...
    try {
        ...
        if (!tokenResponse.ok) {
            console.error('[DEBUG] /google/callback: Failed to get token:', await tokenResponse.text());
            return c.redirect('/?login_error=1');
        }
        const tokenData = await tokenResponse.json();
        console.log('[DEBUG] /google/callback: Got token successfully.');
        // 2. ユーザー情報の取得
        const userResponse = await fetch('https://www.googleapis.com/oauth2/v2/userinfo', {
            headers: { Authorization: `Bearer ${tokenData.access_token}` },
        });
        if (!userResponse.ok) {
            console.error('[DEBUG] /google/callback: Failed to get user info:', await userResponse.text());
            return c.redirect('/?login_error=1');
        }
        const profile = await userResponse.json();
        console.log('[DEBUG] /google/callback: Got user profile:', profile.id);
        // ---
        ...
        if (!existingUser) {
            ...
            console.log('[DEBUG] /google/callback: Created new user and default group.');
        }
        else {
            const userData = db.firestoreToJson(existingUser);
            userRole = userData.role || 'user';
            console.log('[DEBUG] /google/callback: Existing user found. Role:', userRole);
        }
        // 3. JWTセッションの作成
        let verifiedGroups = [];
        ...
        console.log('[DEBUG] /google/callback: Cookie set. Redirecting to /');
        return c.redirect('/');
    }
    catch (error) {
        console.error('[DEBUG] /google/callback: OAuth Callback Error:', error);
        return c.redirect('/?login_error=1');
    }
});
// ログアウト処理
auth.get('/logout', (c) => {
    console.log('[DEBUG] /auth/logout: Deleting session and verifiedGroups cookies');
...
auth.post('/clear-group-verification', async (c) => {
    try {
        console.log('[DEBUG] /clear-group-verification: Deleting verifiedGroups cookie');
        deleteCookie(c, 'verifiedGroups', { path: '/' });
