...
async function checkGroupAccess(c, db, groupId, groupData) {
    if (!groupData.password)
        return true;
    const verifiedStr = getCookie(c, 'verifiedGroups') || '';
    console.log(`[DEBUG] checkGroupAccess: Checking password for groupId ${groupId}. Cookie verifiedGroups = "${verifiedStr}"`);
    if (verifiedStr.split(',').includes(groupId)) {
        console.log(`[DEBUG] checkGroupAccess: Access GRANTED via verifiedGroups cookie`);
        return true;
    }
    ...
    if (authToken && memberId) {
        ...
        if (memberDoc && db.firestoreToJson(memberDoc).deleteToken === authToken) {
            console.log(`[DEBUG] checkGroupAccess: Access GRANTED via participant token`);
            return true;
        }
    }
    console.log(`[DEBUG] checkGroupAccess: Access DENIED.`);
    return false;
}
