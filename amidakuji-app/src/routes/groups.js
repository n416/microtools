...
groups.put('/groups/:groupId/settings', requireAuth, async (c) => {
    try {
        ...
        if (settings.name && settings.name.trim())
            updatePayload.name = settings.name.trim();
        if (settings.customUrl !== undefined)
            updatePayload.customUrl = settings.customUrl.trim();
        if (settings.noIndex !== undefined)
            updatePayload.noIndex = !!settings.noIndex;
...
groups.post('/groups/:groupId/login-or-register', async (c) => {
    try {
        ...
        const existing = (results || []).find((r) => r.document);
        if (existing) {
            ...
            if (data.password) {
                ...
                if (!match)
                    return c.json({ error: 'Invalid password', requiresPassword: true }, 401);
            }
...
groups.post('/groups/:groupId/verify-password', async (c) => {
    try {
        ...
        if (match) {
            ...
            console.log(`[DEBUG] /verify-password: Password matched for ${groupId}. Setting verifiedGroups = "${Array.from(verifiedGroups).join(',')}"`);
...
groups.delete('/prize-masters/:masterId', requireAuth, async (c) => {
    try {
        ...
        if (!groupDoc)
            return c.json({ error: 'Group not found' }, 404);
        if (db.firestoreToJson(groupDoc).ownerId !== user.targetUserId)
            return c.json({ error: 'Permission denied' }, 403);
...
groups.post('/groups/:groupId/cleanup-events', requireAuth, async (c) => {
    try {
        ...
        const existingMemberIds = new Set((membersRes || []).filter((r) => r.document).map((r) => {
        ...
        for (const r of (eventsRes || [])) {
            ...
            const newParticipants = (eventData.participants || []).map((p) => {
