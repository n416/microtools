...
export function renderAdminLists(requests, groupAdmins, systemAdmins, handlers) {
    ...
    if (pendingRequestsList) {
        pendingRequestsList.innerHTML = requests.length === 0 
            ? '<li>現在、承認待ちの申請はありません。</li>'
            : requests.map(req => `<li class="item-list-item"><span>${req.name} (${req.email})</span><div class="item-buttons"><button class="approve-btn" data-request-id="${req.id}">承認</button></div></li>`).join('');
    ...
    if (systemAdminList) {
        systemAdminList.innerHTML = systemAdmins.map(admin => {
