...
try {
    ...
    for (const file of validFiles) {
        ...
        linesOut.push(`${file} - 文字数:${charCount}, テキスト行:${textLineCount}, 空行:${emptyLineCount}, 空行率:${rt.toFixed(1)}%, 1行平均文字数:${chL.toFixed(1)}`);
    }
    ...
    linesOut.push(`1行平均文字数: ${allChL.toFixed(1)}`);
    linesOut.push('');

    fs.writeFileSync(path.join(__dirname, 'measure_result.log'), linesOut.join('\n'), 'utf8');
    fs.writeFileSync(path.join(__dirname, 'measure_summary.txt'), linesOut.join('\n'), 'utf8');
} catch (e) {
    fs.writeFileSync(path.join(__dirname, 'measure_error.log'), e.toString(), 'utf8');
}
