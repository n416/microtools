...
let currentConfirmExecuteCallback = null;

function showConfirmModal(message, onConfirm) {
  if (!confirmModal || !confirmModalMessage) return;
  confirmModalMessage.textContent = message;
  currentConfirmExecuteCallback = onConfirm;
...
function hideConfirmModal() {
  ...
  currentConfirmExecuteCallback = null;
}

if (btnCloseConfirmModal) btnCloseConfirmModal.addEventListener('click', hideConfirmModal);
...
if (btnExecuteConfirm) {
  btnExecuteConfirm.addEventListener('click', () => {
    const callback = currentConfirmExecuteCallback;
    hideConfirmModal();
...
if (btnBulkNarou) btnBulkNarou.addEventListener('click', () => triggerBulkSync('narou'));
if (btnBulkKakuyomu) btnBulkKakuyomu.addEventListener('click', () => triggerBulkSync('kakuyomu'));

function triggerBulkSync(platform) {
  const checkboxes = document.querySelectorAll('.row-checkbox:checked');
  if (checkboxes.length === 0) {
    showMessage('error', '同期するエピソードが選択されていません。');
    return;
  }
  
  const platformName = platform === 'narou' ? '小説家になろう' : 'カクヨム';
  const msg = `チェックされた ${checkboxes.length} 件のエピソードを、「${platformName}」へ【差分確認なしで】順次上書き投稿します。\n本当によろしいですか？`;
  
  showConfirmModal(msg, () => {
    bulkSync(platform, checkboxes);
  });
}

async function bulkSync(platform, checkboxes) {
  ...
  for (let i = 0; i < checkboxes.length; i++) {
    ...
    if (btn) btn.textContent = `一括同期中 (${i+1}/${checkboxes.length}): ${fileName}...`;
