...
function startMirrorCopyMode() {
    const target = transformControls.object;
    if (!target) { log('コピーするオブジェクトが選択されていません。'); return; }
...
class Command { constructor() { this.message = 'コマンド'; } execute() {} undo() {} }
class AddObjectCommand extends Command {
  constructor(object, isPaste = false) { super(); this.object = object; this.isPaste = isPaste; this.message = isPaste ? `オブジェクトをペースト` : `${object.geometry.type.replace('Geometry', '')} を追加`; }
  execute() { mechaGroup.add(this.object); }
  undo() { mechaGroup.remove(this.object); }
}
class DeleteObjectCommand extends Command {
  constructor(object) { super(); this.object = object; this.message = `${object.geometry.type.replace('Geometry', '')} を削除`; }
  execute() { transformControls.detach(); mechaGroup.remove(this.object); }
  undo() { mechaGroup.add(this.object); }
}
class TransformCommand extends Command {
  constructor(object, oldTransform, newTransform) { super(); this.object = object; this.oldTransform = oldTransform; this.newTransform = newTransform; this.message = 'オブジェクトを変形'; }
  execute() { this.object.position.copy(this.newTransform.position); this.object.rotation.copy(this.newTransform.rotation); this.object.scale.copy(this.newTransform.scale); }
  undo() { this.object.position.copy(this.oldTransform.position); this.object.rotation.copy(this.oldTransform.rotation); this.object.scale.copy(this.oldTransform.scale); }
}
class MirrorCopyCommand extends Command {
  constructor(newObject, axis) { super(); this.object = newObject; this.message = `${axis.toUpperCase()}軸に鏡面コピー`; }
  execute() { mechaGroup.add(this.object); }
  undo() { mechaGroup.remove(this.object); }
}
...
fileInput.addEventListener('change', (e) => {
  ...
  reader.onload = (re) => { try { loadFromData(JSON.parse(re.target.result)); } catch (err) { log('ファイル読込失敗'); } };
...
window.addEventListener('pointerdown', (event) => {
  ...
  if (handleIntersects.length > 0) {
    ...
    else if (gizmoMode === 'rotate') { if (hitHandle.name.includes('center')) return; isRotatingIn2DView = true; }
...
window.addEventListener('pointermove', (event) => {
  ...
  if (isDraggingIn2DView || isScalingIn2DView) {
    ...
    switch (draggedInfo.viewportKey) {
      case 'top': newRotation.y = dragStartObjectState.rotation.y + deltaAngle; break;
      case 'front': newRotation.z = dragStartObjectState.rotation.z + deltaAngle; break;
      case 'side': newRotation.x = dragStartObjectState.rotation.x + deltaAngle; break;
    }
...
window.addEventListener('pointerup', (event) => {
  if (isMirrorCopyMode) {
    ...
    for (const key in viewports) { const rect = viewports[key].element.getBoundingClientRect(); if (event.clientX >= rect.left && event.clientX <= rect.right && event.clientY >= rect.top && event.clientY <= rect.bottom) { clickedViewportKey = key; clickedRect = rect; break; } }
  ...
  if (intersects.length > 0) { transformControls.attach(intersects[0].object); log(`選択中: ${intersects[0].object.geometry.type.replace('Geometry', '')}`); } 
  else { transformControls.detach(); log('待機中'); }
});
...
window.addEventListener('keydown', (e) => {
  ...
  if (e.ctrlKey || e.metaKey) {
    switch(e.key.toLowerCase()){
        ...
        case 'y': e.preventDefault(); history.redo(); return;
  ...
  if (isMirrorCopyMode && e.key === 'Escape') { cancelMirrorCopyMode(); return; }
  if (isPasteMode && e.key === 'Escape') { cancelPasteMode(); return; }

  switch (e.key.toLowerCase()) {
    case 't': transformControls.setMode('translate'); log('モード -> 移動 (3Dビュー)'); break;
    case 'r': transformControls.setMode('rotate'); gizmoMode = 'rotate'; updateGizmoAppearance(); log('モード -> 回転 (3D/2Dビュー)'); break;
    case 's': transformControls.setMode('scale'); gizmoMode = 'scale'; updateGizmoAppearance(); log('モード -> 拡縮 (3D/2Dビュー)'); break;
  }

  const selectedObject = transformControls.object;
  if (!selectedObject) return;
  if (e.key === 'Delete' || e.key === 'Backspace') { deleteSelectedObject(); return; }
...
function onWindowResize() {
  for (const key in viewports) {
    ...
    const rect = view.element.getBoundingClientRect();
    const aspect = rect.width / rect.height;
    if (view.camera.isPerspectiveCamera) { view.camera.aspect = aspect; } 
    else { const frustumSize = 10; view.camera.left = -frustumSize * aspect / 2; view.camera.right = frustumSize * aspect / 2; view.camera.top = frustumSize / 2; view.camera.bottom = -frustumSize / 2; }
...
window.addEventListener('load', () => {
  ...
  if (data) { try { loadFromData(JSON.parse(data)); } catch (e) { console.error('自動保存データの復元に失敗', e); } }
  log('初期化完了');
});
...
function animate() {
  ...
  for (const key in viewports) {
    ...
    if (view.camera.isOrthographicCamera) {
      ...
      if (transformControls.object) { renderer.render(scaleGizmoGroup, view.camera); }
