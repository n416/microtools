class Game {
  constructor(canvas, ctx) {
    ...
    this.powerUps = [];
    this.explosions = [];
    this.implosions = []; // ★ 爆縮エフェクト用の配列を追加
  ...
  update(deltaTime) {
    ...
    this.implosions.forEach((imp) => imp.update(deltaTime)); // ★ 爆縮を更新
    ...
    this.implosions = this.implosions.filter((imp) => imp.isActive); // ★ 不要な爆縮を削除
  }
  ...
  draw() {
    ...
    if (this.gameState === 'playing') {
      ...
      this.implosions.forEach((imp) => imp.draw()); // ★ 爆縮を描画
  ...
  addBoss(type) {
    ...
    if (bossData) {
      this.enemies.push(new Boss(this, this.canvas.width, this.canvas.height / 2 - bossData.height / 2, bossData));
    }
  }
