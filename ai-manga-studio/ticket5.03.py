...
# 1. src/types.ts
# VideoBlockにattributesを追加して、演出設定を保存できるようにする
      ...
      storyboard: project.storyboard.map(b => {
        if (b.id === blockId && b.type === 'video') {
          return { 
            ...b, 
            attributes, // 設定を保存
            prompt: prompt || b.prompt // プロンプトも同時に更新可能
          };
        }
        return b;
      }),
...
const projectSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(updateBlockAttributes.fulfilled, (state, action) => updateProjectInState(state, action.payload)) // ★追加
...
# 3. src/components/DirectorsWizard.tsx
# 日本語化、画像DL、Flow追加、状態保存・復元
...
import { saveAs } from 'file-saver'; // 画像DL用
...
interface DirectorsWizardProps {
  ...
  project: Project;
  targetIndex: number;
  getAssetUrl: (id: string | null) => string | undefined; // 画像URL取得用
  onConfirm: (prompt: string, attributes: DirectorAttributes) => void; // attributesも返す
}
...
const DirectorsWizard: React.FC<DirectorsWizardProps> = ({ open, onClose, project, targetIndex, getAssetUrl, onConfirm }) => {
  ...
  }, [open, targetIndex]); // projectは頻繁に変わる可能性があるので依存配列に入れない方が安全だが、currentBlockはprojectから取る
    ...
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      <DialogTitle sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          <Typography variant="h6">演出ウィザード (Director's Wizard)</Typography>
        </Box>
            ...
            <Grid item xs={12} sm={4}>
              <Card variant="outlined" sx={{ borderColor: structure === 'prequel' ? 'primary.main' : 'divider', opacity: canSelectPrequel ? 1 : 0.5 }}>
                <CardActionArea onClick={() => canSelectPrequel && handleStructureSelect('prequel')} disabled={!canSelectPrequel} sx={{ height: '100%', p: 2, textAlign: 'center' }}>
                  ...
                  <Typography variant="h6" fontWeight="bold">前日譚 (Prequel)</Typography>
            ...
            <Grid item xs={12} sm={4}>
              <Card variant="outlined" sx={{ borderColor: structure === 'sequel' ? 'primary.main' : 'divider', opacity: canSelectSequel ? 1 : 0.5 }}>
                <CardActionArea onClick={() => canSelectSequel && handleStructureSelect('sequel')} disabled={!canSelectSequel} sx={{ height: '100%', p: 2, textAlign: 'center' }}>
                  ...
                  <Typography variant="h6" fontWeight="bold">後日談 (Sequel)</Typography>
          ...
          <Box>
            <Accordion defaultExpanded>
              <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                <Typography sx={{ display: 'flex', alignItems: 'center', gap: 1 }}><MovieIcon fontSize="small"/> カメラワーク (Camera Work)</Typography>
              </AccordionSummary>
              <AccordionDetails>
                <Stack spacing={3}>
                  <Box>
                    <Typography variant="caption" color="text.secondary" gutterBottom>カメラの動き (Movement) ※複数可</Typography>
                    {renderTermChips(CAMERA_MOVEMENTS, selectedMovements, (id) => toggleSelection(id, selectedMovements, setSelectedMovements), true)}
                  </Box>
                  <Divider />
                  <Box>
                    <Typography variant="caption" color="text.secondary" gutterBottom>ショットサイズ (Shot Size)</Typography>
                    {renderTermChips(SHOT_SIZES, selectedShotSize, setSelectedShotSize)}
                  </Box>
                  <Box>
                    <Typography variant="caption" color="text.secondary" gutterBottom>アングル (Angle)</Typography>
                    {renderTermChips(ANGLES, selectedAngle, setSelectedAngle)}
                  </Box>
                  <Box>
                    <Typography variant="caption" color="text.secondary" gutterBottom>フォーカス (Focus)</Typography>
            ...
            <Accordion>
              <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                <Typography>雰囲気と感情 (Atmosphere & Emotion)</Typography>
              </AccordionSummary>
              <AccordionDetails>
                <Stack spacing={3}>
                  <Box>
                    <Typography variant="caption" color="text.secondary" gutterBottom>照明と時間 (Lighting & Time)</Typography>
                    {renderTermChips(LIGHTING_TYPES, selectedLighting, setSelectedLighting)}
                  </Box>
                  <Box>
                    <Typography variant="caption" color="text.secondary" gutterBottom>感情 (Emotion)</Typography>
          ...
          <Box>
            ...
            <Box sx={{ mt: 3 }}>
              ...
              <Stack direction="row" spacing={2} flexWrap="wrap" useFlexGap>
                ...
                {/* Flow: 本命ボタン (URL仮置き) */}
                <Button variant="contained" color="secondary" startIcon={<LaunchIcon />} onClick={() => handleCopyAndLaunch('https://flow.art')}>Flow</Button>
              </Stack>
            </Box>
          </Box>
        )}
...
# Wizard呼び出し時のProps修正と、handleWizardConfirmの修正
...
import { ImageBlock, VideoBlock, DirectorAttributes } from '../../types';
      ...
      >
        ...
        <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 1 }}>
           <Box sx={{ display: 'flex', gap: 1 }}>
             ...
             <Tooltip title="演出ウィザードを開く (Director's Wizard)">
               <Button 
        ...
        <Alert onClose={() => setLinkSnackbarOpen(false)} severity="success" sx={{ width: '100%', alignItems: 'center' }} icon={<ContentCopyIcon />}>
            <Box>
                ...
                <Stack direction="row" spacing={1}>
                    ...
                    {/* Flow Link */}
                    <Button size="small" variant="contained" color="secondary" startIcon={<LaunchIcon />} href="https://flow.art" target="_blank">Flow</Button>
                </Stack>
            </Box>
        </Alert>
      </Snackbar>
...
print("\\nTicket #5: Directors Wizard localized & persistence enabled.")